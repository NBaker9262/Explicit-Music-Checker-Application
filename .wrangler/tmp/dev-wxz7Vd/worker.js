...
__name(checkProfanityLocally, "checkProfanityLocally");
...
async function handleAdminSoundCloudResolve(request, env) {
  ...
  if (!searchResult.ok) {
    return json({
      ...
      detail: searchResult.detail || "search_failed",
      query,
      candidates: []
...
var worker_default = {
  async fetch(request, env) {
    ...
    try {
      ...
      if (request.method === "GET" && url.pathname === "/api/admin/soundcloud/resolve") {
        const unauthorized = requireAdmin(request, env);
        if (unauthorized) return withCors(unauthorized, corsHeaders);
...
// .wrangler/tmp/bundle-uaj5RY/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-uaj5RY/middleware-loader.entry.ts
